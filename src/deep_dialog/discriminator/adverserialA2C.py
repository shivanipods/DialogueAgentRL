...
class AdverserialA2C():
	def __init__(self, env, args):
		...
		self.discriminator = Sequential([
			Dense(20, input_shape=(self.nS + 1,), kernel_initializer=kernel),
			Activation('relu'),
			Dense(1, activation='linear', kernel_initializer=kernel),
		])
	...
	def generate_episode(self, env, model, render=False):
		...
		while not is_terminal:
			action_distribution, cricval = self.actor.predict(np.expand_dims(current_state, 0))
	...
	def train(self):
		...
		self.discriminator.compile(optimizer=self.critic_optimizer, loss='categorical_crossentropy')
